...
export default function HostelsPage() {
    return (
        <div className="space-y-12">
                ...
                {[
                    ...
                    <Card key={i} className="border-orange-100 bg-orange-50/50">
                        <CardContent className="p-6 flex flex-col items-center text-center">
                            <stat.icon className="w-8 h-8 text-orange-500 mb-2" />
            ...
            >
                <div className="mb-6">
                    <h2 className="text-2xl font-bold text-slate-900">Residential Blocks</h2>
                    <p className="text-slate-600">Choose a block to view specific details and amenities.</p>
                </div>
                ...
                <Tabs defaultValue="visvesvaraya" className="w-full">
                    <TabsList className="grid w-full grid-cols-2 lg:grid-cols-4 h-auto p-1 bg-slate-100/80 rounded-xl">
                        {hostelBlocks.map((block) => (
                            <TabsTrigger
                                key={block.id}
                                value={block.id}
                                className="h-12 data-[state=active]:bg-white data-[state=active]:text-orange-600 data-[state=active]:shadow-sm rounded-lg"
                            >
                    ...
                    {hostelBlocks.map((block) => (
                        <TabsContent key={block.id} value={block.id} className="mt-6 focus-visible:outline-none">
                            <Card className="border-none shadow-lg overflow-hidden">
                                <div className="grid md:grid-cols-5 bg-white">
                                    <div className="md:col-span-3 p-8">
                                        ...
                                        <h3 className="text-3xl font-bold text-slate-900 mb-4">{block.name}</h3>
                                    ...
                                    <div className="md:col-span-2 bg-slate-50 p-8 border-l border-slate-100 flex flex-col justify-center">
                                        <div className="space-y-6">
                                            <div>
                                                <div className="text-sm text-slate-500 mb-1">Total Capacity</div>
                                                <div className="text-3xl font-bold text-slate-900">{block.capacity}</div>
                                            </div>
                                            <div>
                                                <div className="text-sm text-slate-500 mb-1">Warden Contact</div>
                                ...
                                <div className="mt-6 grid sm:grid-cols-2 gap-4">
                                    {wardenDetails.map((warden, index) => (
                                        <div key={index} className="bg-slate-50 p-4 rounded-lg border border-slate-100">
                                            <div className="text-xs font-bold text-orange-600 uppercase mb-1">{warden.role}</div>
                                            <div className="font-bold text-slate-900">{warden.name}</div>
                                            <div className="text-xs text-slate-500">{warden.designation}</div>
                                            <div className="mt-2 text-sm font-mono text-slate-700">{warden.phone}</div>
                                        </div>
                                    ))}
                                </div>
                            </div>
