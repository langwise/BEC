...
export default function AlumniCatchAll({ params }: { params: Params }) {
  const slug = params.slug ?? [];
  const path = slug.join("/");
  const title = formatTitle(slug) || "Alumni";
      ...
      <header className="border-b border-stone-200 bg-linear-to-br from-orange-50 via-white to-stone-50">
        <div className="container mx-auto max-w-6xl px-4 lg:px-6 py-12 space-y-3">
          ...
          <p className="text-base text-gray-700 leading-relaxed max-w-3xl">
            Placeholder for alumni highlights while detailed content is readied.
          </p>
        </div>
      </header>
        ...
        <Card className="rounded-sm border-stone-200">
          <CardHeader>
            ...
            <CardDescription className="text-sm text-gray-700">
              Alumni content will be published from official sources.
            </CardDescription>
          </CardHeader>
          <CardContent className="text-sm text-gray-700 leading-relaxed">
            You reached: <span className="font-semibold">{`/alumni/${path}`}</span>
          </CardContent>
        </Card>
      </section>
    </main>
