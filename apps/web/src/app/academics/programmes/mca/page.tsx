...
const mcaData = {
  ...
  // Curriculum focus areas drawn from the MCA scheme & syllabus.
  disciplines: [
...
export default function MCAProgrammePage() {
  return (
    <div className="space-y-16">
      ...
      {/* Focus Areas */}
      <section>
          <SectionHeader icon={Code2} title="Curriculum Focus Areas" />
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {mcaData.disciplines.map((dept, idx) => (
                  <FadeIn key={idx} delay={idx * 0.05}>
                    <div className="group p-4 rounded-lg bg-white border border-gray-100 text-gray-800 font-medium hover:border-primary/40 hover:text-primary transition-all duration-300 shadow-sm flex items-center gap-3">
                        <Cpu className="w-4 h-4 text-gray-400 group-hover:text-primary shrink-0" />
                        {dept}
                    </div>
                  </FadeIn>
              ))}
