...
### Missing / to improve 🔧
- **Full faculty rosters + curriculum section — done ✅** (2026-06-07). **(a)** Completed every department's roster from the live `becbgk.edu/<DEPT>/Faculty_<DEPT>.php` pages: added 60 name-only faculty (no doc → initials, no CV modal) on top of the doc-based ones (which keep photo+CV), reconciled by hand to avoid duplicates. **137 faculty** total. **(b)** New **"Curriculum & Syllabus"** section (`documents` content field → `type: "documents"` section rendering R2 PDF download links): wired the updated Scheme & Syllabus / BoS PDFs from `data/new-provided/` for CSE, Civil, MBA and MCA (MCA `.docx`→PDF), uploaded to `departments/<assetSlug>/docs/`. **Robustness:** `getDepartmentFaculty` and `resolveDocuments` now only emit a URL when the key exists on R2 (absolute URL) — a missing asset no longer loads our own 404/nav inside the card iframe; the card falls back to initials / no modal.
- **Rich department content rolled out to 10 departments — done ✅** (2026-06-07). Authored `content/departments.json` (overview, vision/mission, programs, PEOs/PSOs, highlights, labs, associations, contact) from the provided `data/new-provided/<DEPT>/` department-profile docs for: CSE, ECE, AIML, Automobile, Civil, EEE, Physics, Chemistry, MCA, MBA. Galleries auto-populate from the `cineXXXX.webp` scene photos already on R2. **Gallery bug fixed:** `getDepartmentGallery` now excludes the `faculty/` subtree (portraits + CV PDFs) and non-image files, so the "Infrastructure & Labs" gallery shows only scene photos. ECS and Maths have faculty but no department-profile doc (faculty + gallery only); Biotech, IP, ISE, Mechanical, Humanities and the PG specializations remain name-only (no source docs).
- **Faculty rolled out to 9 departments — done ✅** (2026-06-07) via `scripts/build-faculty.mjs` (parses name+designation from the provided profile docs, converts each to PDF = the CV, and extracts the embedded portrait → WebP; one doc = one person, no photo↔name mapping). **77 faculty** total: CSE 20, Civil 20, ECE 16, Maths 6, AIML 5, Automobile 3, ECS 3, Physics 3, EEE 1. MBA/MCA/Chemistry have no per-faculty profile docs (dept-level data only). See [faculty-extraction.md](faculty-extraction.md).
- **Remaining departments** still carry only `name` (+ `assetSlug`) — Biotechnology, Industrial & Production, Information Science, Mechanical, Humanities, and the PG civil/mech specializations have no provided faculty docs yet. Dept-level rich content (overview/vision/labs/etc.) is still CSE-only — repeat that pass from `data/new-provided/<DEPT>/`.
